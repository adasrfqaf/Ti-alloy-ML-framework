...
on the C14 Laves phase dataset. No files are saved; all results are printed to console.
"""
...
# GBDT hyperparameter search space (for small dataset)
PARAM_DIST = {
    'n_estimators': [100],
    'learning_rate': [0.11],
    'max_depth': [7],
...
def load_data(path):
    """Load C14 dataset with English column names."""
...
# ====================== Retrain Best Model and Print Predictions ======================
print("\n" + "=" * 120)
print("Retraining Global Best Model and Displaying Predictions (No Files Saved)")
print("=" * 120)
...
print(f"{'True':>8} {'Pred':>8} {'Diff':>8}")
print("-" * 28)
for i in range(min(10, len(y_test))):
    diff = y_test_pred[i] - y_test[i]
    print(f"{y_test[i]:>8.4f} {y_test_pred[i]:>8.4f} {diff:>8.4f}")

print(f"\nPrediction range: [{y_test_pred.min():.4f}, {y_test_pred.max():.4f}]")
print(f"Correlation with true values: {np.corrcoef(y_test, y_test_pred)[0,1]:.4f}")
...
test_results.to_csv("test_predictions_GN_C14.csv", index=False)
print("\n✅ Test predictions saved to: test_predictions_GN_C14.csv")