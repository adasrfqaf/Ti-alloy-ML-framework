"""
Supplementary Code - XGBoost Fine-Tuning with Gaussian Noise Augmentation (TiFe Dataset)

This script performs fine-tuning for XGBoost with Gaussian noise augmentation
...
# XGBoost hyperparameter search space (matching GBDT's conservative approach)
PARAM_DIST = {
    # Tree structure - shallower trees to prevent overfitting
    'max_depth': [6],
    'min_child_weight': [8],

    # Learning - lower learning rate with more trees
    'learning_rate': [0.08],
    'n_estimators': [178],

    # Sampling - more aggressive subsampling
    'subsample': [0.98],
...
# Noise levels and expansion factors (moderate augmentation for small dataset)
SIGMA_RATIOS = [0.055, 0.06]
EXPANSION_FACTORS = [4, 5, 6]
...
# ====================== Main Pipeline ======================
print("=" * 100)
print("XGBoost Fine-Tuning - TiFe Dataset (Gaussian Noise Augmentation)")
print("=" * 100)
...
for sigma_ratio in SIGMA_RATIOS:
    for n_copies in EXPANSION_FACTORS:
        ...
        # XGBoost model
        xgb_model = xgb.XGBRegressor(random_state=RANDOM_STATE, verbosity=0)
        random_search = RandomizedSearchCV(
            xgb_model, PARAM_DIST, n_iter=N_ITER, cv=CV_FOLDS,
...
# ====================== Results Summary ======================
print("\n\n" + "=" * 120)
print("XGBoost Performance Comparison (Different Noise Levels × Expansion Factors)")
print("=" * 120)
...
print("\n" + "=" * 120)
print("XGBoost Optimization Complete! No files were saved.")
print("=" * 120)