"""
Supplementary Code - GBDT Final Model Training with Gaussian Noise + SMOTER Augmentation (C14 Dataset)

This script trains the final GBDT model with optimal parameters on the C14 Laves phase dataset.
"""
...
if __name__ == '__main__':
    ...
    # Cross-validation
    print("\nPerforming 5-fold cross-validation with best parameters...")
    cv_model = GradientBoostingRegressor(**BEST_PARAMS)
    cv_scores = cross_val_score(cv_model, X_aug, y_aug, cv=CV_FOLDS, scoring='r2')
    cv_r2_mean = cv_scores.mean()
    cv_r2_std = cv_scores.std()
    print(f"CV R² = {cv_r2_mean:.4f} (±{cv_r2_std:.4f})")

    # Train final model
    print("\nTraining final GBDT model with best parameters...")
