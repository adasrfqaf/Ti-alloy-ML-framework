...
def manual_smoter_interpolate(X_train, y_train, mode='both', high_ratio=0.1, low_ratio=0.1, k=5):
    """
    Manual SMOTER interpolation (linear interpolation without noise)
    Supports modes: 'high' (augment high-value tail),
    ...
    # Calculate K-nearest neighbors for minority samples
    n_neighbors = min(k, len(minority_idx) - 1)
    if n_neighbors < 1:
        print("  Warning: Too few minority samples, returning original data")
        return X_train, y_train

    nbrs = NearestNeighbors(n_neighbors=n_neighbors, metric='euclidean').fit(X[minority_idx])
...
def evaluate_models(X_train, X_test, y_train, y_test):
    """Evaluate all models, return DataFrame sorted by Test R²"""
...
# ====================== Main Program ======================
print("=" * 70)
print("C14 Alloy Screening Dataset - Manual SMOTER Interpolation (All Models)")
print("=" * 70)
...
# Apply augmentation
print("\nAugmentation strategy: Both tails (top 10% + bottom 10%)")
X_train_aug, y_train_aug = manual_smoter_interpolate(
    X_train, y_train, mode='both', high_ratio=0.1, low_ratio=0.1, k=5
)
...
print("\n" + "=" * 70)
print("🏆 Top 3 Models (sorted by Test R²):")
