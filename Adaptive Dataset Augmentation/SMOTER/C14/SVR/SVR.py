"""
Supplementary Code for C14 Laves Phase - SVR Grid Search Optimization with SMOTER Augmentation

This script performs hyperparameter optimization for SVR with SMOTER augmentation
...
print("=" * 60)
print("C14 Laves Phase - SVR Grid Search")
print("=" * 60)
...
# Standardization (SVR is sensitive to scale)
...
svr_base = SVR(max_iter=5000)
grid_search = GridSearchCV(
    svr_base, param_grid,
    cv=5, scoring='r2',
    n_jobs=1, verbose=0  # 静默
)
...
# ==================== Evaluation ====================
y_train_pred = best_svr.predict(X_train_selected)
y_test_pred = best_svr.predict(X_test_selected)
...
cv_scores_r2 = cross_val_score(best_svr, X_train_selected, y_train_aug, cv=cv, scoring='r2')
