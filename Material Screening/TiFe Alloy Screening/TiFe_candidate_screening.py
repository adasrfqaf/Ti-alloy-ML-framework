"""
TiFe Phase Candidate Screening Script
GBDT Model with SMOGN Augmentation

This script loads the trained optimal model and screens for promising
TiFe alloy compositions within the high-capacity composition window.
"""
...
model_path = "tife_gbdt_smogn_best.pkl"
best_model = joblib.load(model_path)
print(f"Model loaded: {type(best_model).__name__}")

scaler_path = "tife_scaler.pkl"
scaler = joblib.load(scaler_path)
print("Scaler loaded")

features_path = "tife_selected_features.pkl"
selected_features = joblib.load(features_path)
print(f"Selected features loaded: {len(selected_features)} features")

stats_path = "tife_train_stats.pkl"
...
raw_data = pd.read_csv("TiFe_data.csv")
print(f"\nRaw data shape: {raw_data.shape}")

# Extract high-capacity samples (>2.5 wt%)
...
# Based on analysis of high-capacity TiFe samples (>2.5 wt%)
# Key findings from data:
#   - Ti: 20-35%, Fe: 0-25%, V: 0-35%, Zr: 0-35%, Nb: 0-35%, Ni: 0-25%
#   - High capacity (>2.8 wt%) samples: Ti33V33Zr33, Ti33Nb33V33
#   - Window is wider than BCC due to more diverse high-capacity compositions

# 修改元素窗口
ELEMENT_WINDOW = {
    'Element_at_pct_Ti': (20, 50),
    'Element_at_pct_Fe': (20, 55),  # 强制Fe≥20%
    'Element_at_pct_V': (0, 15),
    'Element_at_pct_Zr': (0, 15),
    'Element_at_pct_Nb': (0, 10),
    'Element_at_pct_Ni': (0, 10),
    'Element_at_pct_Mn': (0, 15),
    'Element_at_pct_Cr': (0, 10),
}

# 或者维持原窗口但增加约束
def check_constraints(row):
    fe = row.get('Element_at_pct_Fe', 0)
    if fe < 20 or fe > 55:   # 强制Fe在合理范围内
        return False
    # Ti+Fe ≥ 60%（保证TiFe相主导）
...
# Ce, Co, Cu, Mo, Ta, Y show no positive contribution
EXCLUDED_ELEMENTS = ['Element_at_pct_Ce', 'Element_at_pct_Co', 'Element_at_pct_Cu',
                     'Element_at_pct_Mo', 'Element_at_pct_Ta', 'Element_at_pct_Y']

print("\nComposition window (based on high-capacity TiFe sample analysis):")
...
def check_constraints(row):
    ...
    # Ti + Fe should be at least 30% (TiFe phase stability)
    ti = row.get('Element_at_pct_Ti', 0)
    fe = row.get('Element_at_pct_Fe', 0)
    if ti + fe < 30:
        return False
...
FIXED_PARAMS = {
    'Unit_Cell_Volume_Å3': 27.0,  # TiFe typical value
...
def generate_candidates(element_window, excluded_elements, n_candidates=50000, step=2):
    ...
    while len(candidates) < n_candidates and iteration < max_iterations:
        ...
        # Randomly select values for each element
        combo = []
...
def format_candidate(row, decimals=0):
    ...
    element_order = ['Element_at_pct_Ti', 'Element_at_pct_Fe', 'Element_at_pct_V',
                     'Element_at_pct_Zr', 'Element_at_pct_Nb', 'Element_at_pct_Ni',
                     'Element_at_pct_Mn', 'Element_at_pct_Cr']
    comp_parts = []
...
def is_distinct(candidate, existing_candidates, tol=2.0):
    """Check if candidate is distinct from existing ones."""
    if not existing_candidates:
        return True
    # Only compare key elements for TiFe
    key_elements = ['Element_at_pct_Ti', 'Element_at_pct_Fe', 'Element_at_pct_V',
                    'Element_at_pct_Zr', 'Element_at_pct_Nb']
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("TiFe Phase Candidate Screening")
    print("=" * 70)
    ...
    # Scale features (GBDT was trained on scaled data)
    ...
    for _, row in results.iterrows():
        if is_distinct(row, distinct_results, tol=2.0):
    ...
    # Display results
    print("\n" + "=" * 70)
    print("Top 3 Distinct TiFe Candidates")
    print("=" * 70)

    for idx, row in enumerate(distinct_results, 1):
        comp_str = format_candidate(row, decimals=1)
        details = []
        for col in ['Element_at_pct_Ti', 'Element_at_pct_Fe', 'Element_at_pct_V',
                    'Element_at_pct_Zr', 'Element_at_pct_Nb', 'Element_at_pct_Ni',
                    'Element_at_pct_Mn', 'Element_at_pct_Cr']:
    ...
    # Table 5 format
    print("=" * 70)
    print("Table 5 Format: Top 3 Distinct TiFe Candidates")
    print("=" * 70)
    ...
    for idx, row in enumerate(distinct_results, 1):
        ...
        zr = row.get('Element_at_pct_Zr', 0)
        nb = row.get('Element_at_pct_Nb', 0)
        features = []
        if ti > 0:
        ...
        if zr > 0:
            features.append(f"Zr~{zr:.0f}%")
        if nb > 0:
            features.append(f"Nb~{nb:.0f}%")
        print(f"{idx:<6} {comp_str:<35} {cap:.2f}            {', '.join(features)}")

    # Save
    results.to_csv("tife_candidates.csv", index=False)
    print("\nResults saved to: tife_candidates.csv")
