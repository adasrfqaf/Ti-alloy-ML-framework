# =============================================================================
# C14 Dataset - Data Augmentation Visualization
# Methods: Gaussian Noise (GN), SMOTER, SMOTER+GN, SMOGN
# Output: KDE plot (Fig3) and PCA projection (Fig4)
# Colors: Original #4EABC0, Synthetic #FFA453
# All comments in English, fonts Times New Roman
# =============================================================================
...
# ==================== 1. Load data ====================
file_path = r"C14_data.csv"   # Adjust path if needed
...
def smoter_gaussian_synthetic(X, y, extreme_ratio, noise_std, n_ratio):
    """SMOTER followed by Gaussian noise on the combined (original+SMOTER) set."""
...
# ==================== 7. Fig3: KDE comparison (Original vs GN) ====================
plt.figure(figsize=(10, 7))

sns.kdeplot(y, label='Original', color=COLOR_ORIG, linestyle='-', linewidth=4)
sns.kdeplot(y_gn, label='GN Synthetic', color=COLOR_SYNTH, linestyle='--', linewidth=4)
...
plt.title('C14: Target distribution (Original vs GN)', fontsize=24, fontweight='bold', fontfamily='Times New Roman')
...
plt.tight_layout()
plt.savefig('Fig3_C14_KDE_GN.png', dpi=600, bbox_inches='tight')
plt.show()
...
def plot_pca_projection(X_orig, y_orig, X_synth, y_synth, title, ax):
    ...
    leg = ax.legend(loc='upper right', fontsize=28, frameon=True, shadow=True)  # Legend upper right
    for text in leg.get_texts():
        text.set_fontfamily('Times New Roman')
        text.set_fontweight('bold')
...
for i, ax in enumerate(axes.flat):
    ax.text(0.03, 0.97, f'({chr(97 + i)})', transform=ax.transAxes,
            fontsize=28, fontweight='bold', va='top', fontfamily='Times New Roman')

plt.tight_layout()
plt.savefig('Fig4_C14_PCA_projection.png', dpi=600, bbox_inches='tight')
plt.show()
